...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f"Successfully composed video for job {job_id}: {composed_video_url}")
        ...
        if 'job_id' in locals():
            update_job_coordination_status(locals()['job_id'], 'composition_status', 'failed')
...
def check_both_ready(job_id: str) -> bool:
    ...
    try:
        ...
        video_status = item.get('video_status', {}).get('S', '')
        audio_status = item.get('audio_status', {}).get('S', '')
        
        logger.info(f"Job {job_id} status - Video: {video_status}, Audio: {audio_status}")
        
        return video_status == 'complete' and audio_status == 'complete'
...
def compose_single_scene(audio_file: Dict[str, Any], video_file: Dict[str, Any], scene_index: int) -> str:
