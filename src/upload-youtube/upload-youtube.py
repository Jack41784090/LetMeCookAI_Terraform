...
def prepare_video_metadata(
    ...
    # Use stored video metadata if available
    stored_title = job_details.get("video_title", "")
    stored_summary = job_details.get("video_summary", "")
    stored_hashtags = job_details.get("video_hashtags", "")
    stored_topic = job_details.get("video_topic", "")
...
{stored_hashtags.replace(',', ' ') if stored_hashtags else '#AI #AIGenerated #ArtificialIntelligence #VideoGeneration #AutomatedContent #TechDemo'}
...
"""
    ...
    if stored_hashtags:
        # Convert comma-separated hashtags to list and clean them
        tags = []
        for tag in stored_hashtags.split(','):
            tag = tag.strip()
            if tag.startswith('#'):
